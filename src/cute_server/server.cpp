...
int main(void) 
{
	const char* address_and_port = "127.0.0.1:2048";
	...
	while (1) {
		...
		while (cn_server_pop_event(server, &e)) {
			if (e.type == CN_SERVER_EVENT_TYPE_NEW_CONNECTION) 
			{
				...
				client_indexes[num_clients] = (int)e.u.new_connection.client_index;
				num_clients++;
			} 
			...
			else if (e.type == CN_SERVER_EVENT_TYPE_DISCONNECTED) 
			{
				...
				for (int i = 0; i < num_clients; i++)
				{
					if (client_indexes[i] == e.u.disconnected.client_index)
						index = i;
